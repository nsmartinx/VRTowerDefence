...
public class Units : MonoBehaviour
{
    ...
    //instead of storing the health on the XRRIG, store it in a health manager empty object that stores player 1 and player 2 health. Have each XRRig access that game object and display the health associated with their player number.
    private void Awake()
    {
    ...
    void Start()
    {
        target = Waypoints.waypoints[0];//sets the target to the first waypoint
    ...
    void Update()
    {
        ...
        if (waypointIndex <= Waypoints.waypoints.Length)//if it has not reached the final waypoint
        {
        ...
        {
            PhotonNetwork.Destroy(gameObject);
            healthManager.GetComponent<HealthManager>().damage(damage, 1);
        }
    ...
    void GetNextWaypoint()//sets target to the next waypoint in the list
    {
        ...
        if (waypointIndex < Waypoints.waypoints.Length)//if it has not reached the final waypoint
            target = Waypoints.waypoints[waypointIndex];//finds the next waypoint and sets target to that
    }
